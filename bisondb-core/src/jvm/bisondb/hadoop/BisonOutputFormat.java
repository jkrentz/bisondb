...
import bisondb.persistence.Coordinator;
...
public class BisonOutputFormat implements OutputFormat<IntWritable, BisonRecordWritable> {

    public static Logger LOG = Logger.getLogger(BisonOutputFormat.class);
    public static final String ARGS_CONF = "bison.output.args";
    ...
    public class BisonRecordWriter implements RecordWriter<IntWritable, BisonRecordWritable>, Closeable {

        FileSystem fileSystem;
        Args args;
        Map<Integer, Persistence> lps = new HashMap<Integer, Persistence>();
        Progressable progressable;
        LocalBisonManager localManager;

        int numWritten = 0;
        long lastCheckpoint = System.currentTimeMillis();

        public BisonRecordWriter(Configuration conf, Args args, Progressable progressable)
            throws IOException {
            fileSystem = Utils.getFS(args.outputDirHdfs, conf);
            this.args = args;

            this.progressable = progressable;
            localManager = new LocalBisonManager(fileSystem, args.spec, LocalBisonManager.getTmpDirs(conf));
        }
        ...
        public void write(IntWritable shard, BisonRecordWritable carrier) throws IOException {
    ...
    public RecordWriter<IntWritable, BisonRecordWritable> getRecordWriter
            (FileSystem fs,JobConf conf, String string, Progressable progressable)
            throws IOException {
        return new BisonRecordWriter(conf, (Args) Utils.getObject(conf, ARGS_CONF), progressable);
    }
