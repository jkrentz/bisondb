(ns bisondb.common.config-test
  (:use bisondb.common.config
        bisondb.test.common
        midje.sweet)
...
(defn round-trip-spec
  "Passes the supplied DomainSpec through BisonDB's serialization
...
(fact "Check individual attributes."
  (t/with-fs-tmp [fs tmp]
    ...
    (let [jspec (DomainSpec/readFromFileSystem fs tmp)]
      ...
      (class-name (.getCoordinator jspec)) => "bisondb.persistence.JavaBerkDB"
      (class-name (.getShardScheme jspec)) => "bisondb.partition.HashModScheme"
...
(defn round-trip-conf
  ...
  BisonDB's configuration printer."
  [config]
  (t/with-fs-tmp [fs tmp]
