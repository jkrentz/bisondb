(ns bisondb.hadoop.output-format-test
  (:use midje.sweet
        bisondb.test.common
        bisondb.test.keyval)
  ...
  (t/with-fs-tmp [fs output-dir]
    (t/with-local-tmp [lfs etmp tmp2]
      (let [data {0 {(.getBytes "0a") (.getBytes "00")
        ...
        (with-open [writer (bison-writer test-spec
                                            output-dir
                                            etmp)]
