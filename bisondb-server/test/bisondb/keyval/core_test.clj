(ns bisondb.keyval.core-test
  (:use bisondb.keyval.core
        bisondb.test.common
        bisondb.test.keyval
        midje.sweet
        [bisondb.keyval.domain :only (to-map)])
  (:require [jackknife.core :as u]
            [jackknife.seq :as seq]
            [bisondb.common.status :as status]
            [bisondb.common.domain :as dom]
            [bisondb.common.database :as db])
  (:import [bisondb.persistence JavaBerkDB]
  ...
  (with-sharded-domain ["test1"
                        ...
                        {0 {(barr 0) (barr 0 0)
                         ...
                         2 {20 [20 0]
                            21 [21 1]}
                         3 {30 [30 0]}}]
    (with-service-handler [bison
                           [(u/local-hostname) "host2"]
                           {"test1" dpath}
                           {(u/local-hostname) [0 2] "host2" [1 3]}]
      (expected-domain-data bison "test1"
                            0 [0 0]
                            20 [20 0]
                            2 nil)
      (direct-get-val bison "test1" (barr 10)) => (throws Exception))))
  ...
  (with-local-tmp [lfs local-dir]
    (t/with-fs-tmp [fs dtmp1 dtmp2 gtmp]
      (let [domain-spec {:num-shards 4 :coordinator (JavaBerkDB.)}
        ...
        ;; start bdb and shutdown right away to get version 1 of both
        ;; domains
        ...
        ;; remove the local version of it too (delete dir), when starting up bdb
    ...
    (with-presharded-domain ["test1" dpath (JavaBerkDB.) shards-to-pairs]
      (with-service-handler [bison
        ...
        (with-mocked-remote [domain-to-host-to-shards shards-to-pairs ["host4"]]
          (expected-domain-data bison
                                "test1"
                                0 [0 0]
                                20 [20 0]
                                2 nil)
          (facts
            (multi-get-vals bison "test1" [0 22 30]) => [[0 0] nil [30 0]]
            (multi-get-vals bison "test1" [0 1 2 30 10]) => [[0 0] [1 1] nil [30 0] [10 0]]
            (multi-get-vals bison "test1" [])) => [])
        (with-mocked-remote
          [domain-to-host-to-shards shards-to-pairs ["host3" "host4"]]
          (fact
            (multi-get-vals bison "test1" [0 10]) => [[0 0] [10 0]])
          (multi-get-vals bison "test1" [0 22]) => (throws Exception))))))
