(ns bisondb.common.domain
  ...
  (:require [hadoop-util.core :as h]
            ...
            [bisondb.common.shard :as shard]
            [bisondb.common.config :as conf]
            [bisondb.common.status :as status]
            [bisondb.common.thread-pool :as t])
  (:import [bisondb Utils DomainSpec]
           [bisondb.store DomainStore]
           [bisondb.common.status IStateful IStatus KeywordStatus]
           [bisondb.persistence ShardSet Shutdownable]
  ...
  (let [local-store (.localStore domain)
        ...
        open!       (fn [idx]
                      (try
                        ...
                        (catch Throwable e
                          (log/error (format "Error opening shard %s of version %s for domain %s: %s" idx version domain e))
                          nil)))]
    (assert (has-version? local-store version)
            (str version "  doesn't exist."))
    (u/with-ret (let [lps (map open! shards)]
                  (if (some nil? lps)
                    (do
                      (map close-shard! (filter nil? lps))
                      (u/throw-runtime (format "Error retrieving shards for version %s" version)))
                    (zipmap shards lps)))
