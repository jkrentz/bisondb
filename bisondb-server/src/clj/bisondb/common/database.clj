(ns bisondb.common.database
  ...
  (:import [bisondb.persistence Shutdownable]
           [java.io File]))
...
(defn build-meters [domain-name]
  (let [domain-name (clojure.string/replace domain-name #"-" "_")
    ...
    {:direct-get-response-time (timer [(str hostname ".bisondb.domain") domain-name "direct_get_response_time"])
     :multi-get-response-time (timer [(str hostname ".bisondb.domain") domain-name "multi_get_response_time"])}))
...
(defn- domain-path
  ...
  supplied name located within the supplied bisondb root
  directory."
...
(defrecord Database [local-root port domains metrics options]
  ...
  Shutdownable
  (shutdown [_]
    (log/info "BisonDB received shutdown notice...")
