(ns bisondb.test.common
  ...
  (:import [java.nio ByteBuffer]
           ...
           [bisondb.persistence ShardSet]
           [bisondb Utils DomainSpec]
           [org.apache.hadoop.io IntWritable]
           [bisondb.hadoop BisonOutputFormat
            BisonOutputFormat$Args LocalBisonManager]
...
(defn bison-writer
  ...
  (let [args  (BisonOutputFormat$Args. spec output-dir)]
    (when indexer
      (set! (.indexer args) indexer))
    (when update-dir
      (set! (.updateDirHdfs args) update-dir))
    (.getRecordWriter (BisonOutputFormat.)
                      nil
                      (doto (JobConf.)
                        (Utils/setObject BisonOutputFormat/ARGS_CONF args)
                        (LocalBisonManager/setTmpDirs [tmp-dir]))
                      nil
                      nil)))
  ...
  (t/with-local-tmp [_ tmp]
    (let [store         (DomainStore. path spec)
          version-path  (create-version store
                                        :version version
                                        :force? true)]
      (with-open [writer (bison-writer spec path tmp)]
