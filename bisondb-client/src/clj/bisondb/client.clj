...
(defn kv-client [transport]
  (BisonDB$Client. (TBinaryProtocol. transport)))
...
(defmacro with-bison
  "Calls to BisonDB should be wrapped in this macro
  ...
  `(with-open [^TFramedTransport conn# (doto ^TFramedTransport (thrift-transport ~host ~port)
                                         (.open))]
     (let [^BisonDB$Client ~client-sym (kv-client conn#)]
       ~@body)))

(defn get
  "Makes a `get` call to BisonDB and unwraps the thrfit Value struct,
...
(defn multi-get
  "Makes a `multi-get` call to BisonDB. The result map is transformed
